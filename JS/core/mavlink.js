...
const CRC_EXTRA = {
  ...
  40:230, 43:132, 44:221, 47:153, 51:196, 73:38,
...
if(id === 0){ // HEARTBEAT — 本项目约定线序（与 MAVLink common.xml 标准字段顺序不同）：0~3 custom_mode LE, 4 type, 5 autopilot, 6 base_mode, 7 system_status, 8 mavlink_version
  ...
  if (id === 73 && window._missionTransfer && window._missionTransfer.mode === "download") {
    if (window.MavlinkMission && window.MavlinkMission.parseMissionItemInt) {
      const item = window.MavlinkMission.parseMissionItemInt(
  ...
  if (id === 47 && window._missionTransfer && window._missionTransfer.mode === "upload") {
    ...
    if (window._missionTransfer) {
      window._missionTransfer.ack = true;
      if (result !== 0) {
        window._missionTransfer.error = "MISSION_ACK type=" + result;
      }
    }
  }
